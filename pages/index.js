...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (!mounted) return;
    Promise.all([api.dashboard(), api.alerts.list(), api.stats(), api.interventions.list("?limit=5")])
  .then(([d, a, s, iv]) => { setData(d); setAlerts(a); setStats(s); setRecentInterventions(iv.slice(0,5)); })
      .catch(console.error)
    ...
    : recentInterventions.map(i => (
      <div key={i.id} className="tl-item">
        ...
        <div style={{flex:1}}>
          ...
          <div className="tl-meta">{i.department_name||i.site_name||"—"} · {fmtDate(i.date)}</div>
        </div>
      </div>
    ))
  }
