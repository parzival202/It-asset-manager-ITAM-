...
export default async function handler(req,res) {
  ...
  await db.execute("BEGIN"); try { await db.execute({sql:"UPDATE consumables SET stock_qty=stock_qty+?,updated_at=datetime('now') WHERE id=?",args:[qty,id]}); await db.execute({sql:"INSERT INTO consumable_movements (consumable_id,movement_type,quantity,note) VALUES (?,?,?,?)",args:[id,"in",qty,req.body.note||"Entrée manuelle"]}); await db.execute("COMMIT"); const row=await db.execute({sql:"SELECT * FROM consumables WHERE id=?",args:[id]}); return res.json(row.rows[0]); } catch(e){await db.execute("ROLLBACK");return res.status(500).json({error:e.message});}
}
