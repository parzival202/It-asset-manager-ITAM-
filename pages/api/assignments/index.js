...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    const {
      asset_id, user_name, user_title,
      site_id, department_id,
      started_at, ended_at, reason, assigned_by
    } = req.body;
    ...
    try {
      // Clôturer l'affectation active si elle existe
      await db.execute({
        sql: `UPDATE assignments SET ended_at = ? WHERE asset_id = ? AND ended_at IS NULL`,
        args: [started_at, Number(asset_id)]
      });

      // Créer la nouvelle affectation
      const r = await db.execute({
        sql: `INSERT INTO assignments
              (asset_id, user_name, user_title, site_id, department_id,
               started_at, ended_at, reason, assigned_by)
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        args: [
          Number(asset_id),
      ...
      await db.execute({
        sql: `UPDATE assets
              ...
              WHERE id = ?`,
        args: [n(user_name), n(user_title), Number(asset_id)]
      });
