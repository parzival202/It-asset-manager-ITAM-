...
export async function initDb() {
  ...
  // Migration de référentiel : nouveau service du site Cosmétique.
  await db.execute(`
    INSERT INTO departments (site_id, name)
    SELECT s.id, 'Magasin Matière Premièreet'
    FROM sites s
    WHERE s.name = 'Cosmétique'
      AND NOT EXISTS (
        SELECT 1 FROM departments d
        WHERE d.site_id = s.id AND d.name = 'Magasin Matière Premièreet'
      )
  `).catch(()=>{});
...
export async function seedDb() {
  ...
  await db.execute({ sql: "INSERT INTO departments (site_id, name) VALUES (?,?)", args: [siteC, "Magasin Matière Premièreet"] });
