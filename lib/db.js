...
export async function initDb() {
  ...
  performed_by     TEXT,
  intervention_type TEXT NOT NULL DEFAULT 'dépannage',
  asset_status     TEXT,
  ...
  await db.execute(`ALTER TABLE interventions ADD COLUMN intervention_type TEXT DEFAULT 'dépannage'`).catch(()=>{});
  await db.execute(`ALTER TABLE interventions ADD COLUMN asset_status TEXT`).catch(()=>{});
  await db.execute(`ALTER TABLE interventions ADD COLUMN maintenance_id INTEGER REFERENCES maintenances(id)`).catch(()=>{});
  await db.execute(`ALTER TABLE maintenances ADD COLUMN intervention_id INTEGER REFERENCES interventions(id)`).catch(()=>{});
  // Migration de référentiel : corrige le service Cosmétique et conserve les
  // rattachements créés avec les anciens libellés.
  await db.execute(`
    INSERT INTO departments (site_id, name)
    SELECT s.id, 'Magasin Matière Première'
    FROM sites s
    WHERE s.name = 'Cosmétique'
      AND NOT EXISTS (
        SELECT 1 FROM departments d
        WHERE d.site_id = s.id AND d.name = 'Magasin Matière Première'
      )
  `);
  const canonicalDept = await db.execute(`SELECT d.id FROM departments d JOIN sites s ON s.id=d.site_id WHERE s.name='Cosmétique' AND d.name='Magasin Matière Première' LIMIT 1`);
  if (canonicalDept.rows[0]) {
    const canonicalId = Number(canonicalDept.rows[0].id);
    const aliases = `SELECT d.id FROM departments d JOIN sites s ON s.id=d.site_id WHERE s.name='Cosmétique' AND d.name IN ('Magasin Matière Premièreet','Magasin Matière première ')`;
    await db.execute({sql:`UPDATE assets SET department_id=? WHERE department_id IN (${aliases})`,args:[canonicalId]});
    await db.execute({sql:`UPDATE interventions SET department_id=? WHERE department_id IN (${aliases})`,args:[canonicalId]});
    await db.execute({sql:`UPDATE maintenances SET department_id=? WHERE department_id IN (${aliases})`,args:[canonicalId]});
    await db.execute({sql:`UPDATE assignments SET department_id=? WHERE department_id IN (${aliases})`,args:[canonicalId]});
    await db.execute(`DELETE FROM departments WHERE id IN (${aliases})`);
  }
  await db.execute(`
    INSERT INTO departments (site_id, name)
    SELECT s.id, 'Multimédia'
    FROM sites s
    WHERE s.name = 'Cosmétique'
      AND NOT EXISTS (
        SELECT 1 FROM departments d
        WHERE d.site_id = s.id AND d.name = 'Multimédia'
      )
  `);
  // Stock des consommables et historique inviolable des mouvements.
  await db.execute(`CREATE TABLE IF NOT EXISTS consumables (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    reference TEXT NOT NULL UNIQUE,
    name TEXT NOT NULL,
    category TEXT NOT NULL DEFAULT 'toner',
    cartridge_type TEXT NOT NULL DEFAULT 'monochrome',
    color TEXT,
    color_stock TEXT,
  ...
  await db.execute(`CREATE TABLE IF NOT EXISTS consumable_movements (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  await db.execute(`CREATE TABLE IF NOT EXISTS intervention_consumables (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    created_at TEXT DEFAULT (datetime('now')),
    UNIQUE(intervention_id, consumable_id)
  )`);
  try { await db.execute("ALTER TABLE intervention_consumables ADD COLUMN color_quantities TEXT"); } catch (_) {}
  try { await db.execute("ALTER TABLE consumable_movements ADD COLUMN asset_id INTEGER REFERENCES assets(id)"); } catch (_) {}
  try { await db.execute("ALTER TABLE consumable_movements ADD COLUMN department_id INTEGER REFERENCES departments(id)"); } catch (_) {}
  try { await db.execute("ALTER TABLE consumable_movements ADD COLUMN intervention_id INTEGER REFERENCES interventions(id)"); } catch (_) {}
  try { await db.execute("ALTER TABLE consumable_movements ADD COLUMN color_quantities TEXT"); } catch (_) {}
  // Colonnes ajoutées après la création initiale de la base.
  try { await db.execute("ALTER TABLE consumables ADD COLUMN cartridge_type TEXT NOT NULL DEFAULT 'monochrome'"); } catch (_) {}
  try { await db.execute("ALTER TABLE consumables ADD COLUMN color_stock TEXT"); } catch (_) {}
}
...
export async function seedDb() {
  ...
  await db.execute({ sql: "INSERT INTO departments (site_id, name) VALUES (?,?)", args: [siteC, "Multimédia"] });
