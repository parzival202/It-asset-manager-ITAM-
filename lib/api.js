...
export const api = {
  ...
  consumables: { list:()=>req("GET","/consumables"), get:(id)=>req("GET",`/consumables/${id}`), create:(d)=>req("POST","/consumables",d), update:(id,d)=>req("PUT",`/consumables/${id}`,d), delete:(id)=>req("DELETE",`/consumables/${id}`), entry:(id,d)=>req("POST",`/consumables/${id}/movements`,d), report:(q="")=>req("GET",`/consumables/report${q}`) },
  schedule: {
