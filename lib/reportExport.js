...
export function printReport(title, headers, rows, subtitle="") {
  ...
  // `noopener` peut faire retourner null dans Chromium même si la fenêtre vient
  // d'être ouverte. On ouvre donc depuis le geste utilisateur, puis on coupe la
  // référence vers la page d'origine juste après.
  const win=window.open("","_blank"); if(!win) return alert("Autorisez les fenêtres contextuelles pour générer le PDF.");
