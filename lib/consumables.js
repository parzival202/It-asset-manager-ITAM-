...
export async function syncInterventionConsumables(db, interventionId, status, items = []) {
  ...
  const intervention = await db.execute({sql:"SELECT asset_id, department_id FROM interventions WHERE id=?",args:[Number(interventionId)]});
  const assetId = intervention.rows[0]?.asset_id || null;
  const departmentId = intervention.rows[0]?.department_id || null;
  const wanted = status === "done" ? items.filter(x=>Number(x.consumable_id)>0 && (colorTotal(x.color_quantities)>0 || (!x.color_quantities && Number(x.quantity)>0))) : [];
  ...
  for (const item of previous.rows) {
    await db.execute({sql:"INSERT INTO consumable_movements (consumable_id,movement_type,quantity,note,asset_id,department_id,intervention_id) VALUES (?,?,?,?,?,?,?)",args:[Number(item.consumable_id),"in",Number(item.quantity || colorTotal(item.color_quantities)),"Annulation de l’utilisation précédente",assetId,departmentId,Number(interventionId)]});
  }
  ...
  for (const item of wanted) {
    ...
    await db.execute({sql:"INSERT INTO consumable_movements (consumable_id,movement_type,quantity,note,asset_id,department_id,intervention_id) VALUES (?,?,?,?,?,?,?)",args:[Number(item.consumable_id),"out",quantity,"Utilisation dans une intervention",assetId,departmentId,Number(interventionId)]});
  }
}
