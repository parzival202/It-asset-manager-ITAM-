...
export async function syncInterventionConsumables(db, interventionId, status, items = []) {
  ...
  for (const item of wanted) {
    ...
    await db.execute({sql:"INSERT INTO consumable_movements (consumable_id,movement_type,quantity,note,asset_id,department_id,intervention_id,created_at) VALUES (?,?,?,?,?,?,?,?)",args:[Number(item.consumable_id),"out",quantity,"Utilisation dans une intervention",assetId,departmentId,Number(interventionId),interventionDate]});
  }
}
