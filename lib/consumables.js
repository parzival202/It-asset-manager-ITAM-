...
export async function syncInterventionConsumables(db, interventionId, status, items = []) {
  ...
  for (const item of wanted) await db.execute({sql:"INSERT INTO intervention_consumables (intervention_id,consumable_id,quantity,color_quantities) VALUES (?,?,?,?)",args:[Number(interventionId),Number(item.consumable_id),Number(item.quantity || colorTotal(item.color_quantities)),Object.keys(parseColors(item.color_quantities)).length ? JSON.stringify(parseColors(item.color_quantities)) : null]});
}
